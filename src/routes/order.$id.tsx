...
function OrderCheckoutPage() {
  ...
  const submit = useSubmitPayment(id);
  ...
  const send = () => {
    ...
    submit.mutate(
      { userId, file: method === "slip" ? file : null, method, shipping: { name, phone, address } },
      {
        onSuccess: () => {
          setDone(true);
          toast.success("ส่งข้อมูลการชำระเงินแล้ว รอผู้ดูแลตรวจสอบ");
        },
          ...
          <>
            ...
            {done ? (
              <section className="rounded-3xl border border-border/70 bg-card p-8 text-center">
                ...
                <p className="mt-3 font-display text-lg font-semibold">ส่งข้อมูลการชำระเงินแล้ว</p>
                <p className="mt-2 text-sm text-muted-foreground">
                  ทีมงานจะตรวจสอบและยืนยันคำสั่งซื้อของคุณโดยเร็ว
                </p>
                <Link
                  to="/profile"
