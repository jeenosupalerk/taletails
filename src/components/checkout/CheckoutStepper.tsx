...
const steps = [
  ...
  { id: 2, label: "สแกนชำระเงิน", icon: QrCode },
...
export function CheckoutStepper({ current }: { current: 1 | 2 | 3 }) {
  return (
    <ol className="flex items-start gap-1.5 rounded-3xl border border-border/70 bg-card/80 px-3 py-4 backdrop-blur sm:gap-2 sm:px-4">
      {steps.map((s, i) => {
        ...
        return (
          <li key={s.id} className="flex min-w-0 flex-1 items-start gap-1.5 sm:gap-2">
            <div className="flex min-w-0 flex-1 flex-col items-center gap-1.5">
              ...
              <span
                className={cn(
                  "text-center text-[10px] font-medium leading-tight transition-colors duration-500 sm:text-xs",
                  active
            ...
            {!isLast && (
              <span className="mt-4 h-1.5 flex-1 overflow-hidden rounded-full bg-foreground/10 sm:mt-5">
                <motion.span
